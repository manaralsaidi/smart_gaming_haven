    ...
    <div>
      <div className="flex flex-col gap-6 mt-14">
        <div className="w-full justify-between flex items-center">
          <Heading text={title} />
          <Link className="text-teal-400 hover:text-teal-300 font-semibold transition-colors duration-150 drop-shadow-[0_0_5px_rgba(45,212,191,0.3)]" href="/category">
            Browse All Games
          </Link>
        </div>
        <SwiperCards 
          ...
          items={games.map((game: Game) => {
            return {
              card: big ? (
                ...
                <Link href={`/game/${game.id}`} className="flex overflow-hidden items-center bg-main rounded-2xl group transition-all duration-200">
                ...
                <div className="relative group">
                  ...
                  <Link href={`/game/${game.id}`} className="block cursor-pointer">
                    <div
