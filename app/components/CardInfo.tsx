import { Button } from "@/components/ui/button";
import Image from "next/image";
import React from "react";
import MotionItem from "./defaults/MotionItem";

const CardInfo = ({
  desc,
  title,
  image,
  textBtn,
  btnClasses,
}: {
  desc: string;
  title: string;
  image: string;
  textBtn?: string;
  btnClasses?: string;
}) => {
  console.log(desc);
  return (
    <MotionItem
      initial={{ opacity: 0, y: 20 }}
      whileInView={{ opacity: 1, y: 0, transition: { duration: 1 } }}
      className="flex flex-col items-start absolute left-20 top-20 max-w-md"
    >
      <div className="w-96 h-40 relative">
        <Image src={image} fill alt={`${title}`} className="object-contain" />
      </div>
      <h1 className="text-white text-2xl font-semibold">{title}</h1>
      <p className="text-base text-gray-200">{desc}</p>
      
      <Button 
        className={`rounded-full mt-5 bg-teal-500 text-neutral-950 font-bold hover:bg-teal-400 border border-teal-300/30 shadow-[0_0_15px_rgba(45,212,191,0.4)] transition-all duration-200 ${btnClasses || ""}`}
      >
        {textBtn || "Find out more !"}
      </Button>
    </MotionItem>
  );
};

export default CardInfo;