...
import ReviewReply from "@/app/models/reply";
...
// 4. فعل إضافة رد على مراجعة معينة
export async function createReplyAction(reviewId: string, userId: string, content: string) {
  try {
    await connectDB();

    if (!reviewId || !userId || !content?.trim()) {
      return { error: "Review ID, User ID and content are required" };
    }
...
// 5. فعل جلب الردود الخاصة بمراجعة معينة
export async function getRepliesForReview(reviewId: string) {
  try {
    await connectDB();
    ...
    return replies.map((rep) => ({
      id: rep?._id?.toString() || Math.random().toString(),
      content: rep?.content || "",
      createdAt: rep?.createdAt ? new Date(rep.createdAt).toLocaleDateString() : new Date().toLocaleDateString(),
      userName: (rep?.userId as any)?.name || "Anonymous Gamer",
    }));
  } catch (err) {
    console.error("Database error in getRepliesForReview:", err);
    return [];
  }
}
